...
def serch(img):
    ...
    results = {}
    # 책 커버 보관 디렉토리 경로 ---⑤
    cover_path = glob.glob('../img/coins/*.*')
    for cover_path in cover_path:
        ...
        cover_d = cv2.drawKeypoints(cover, kp2, None)
...
for i, label in enumerate(coin_label):
    ...
    if len(results) == 0 :
        ...
        for( i, (accuracy, cover_path)) in enumerate(results):
            ...
            if i==0:
                cover = cv2.imread(cover_path)
                cv2.putText(cover, ("Accuracy:%.2f%%"%(accuracy*100)), (10,100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2, cv2.LINE_AA)
